...
namespace big
{
	...
	bool gta_data_service::is_cache_up_to_date()
	{
		...
		const auto file_version = memory::module("").timestamp();
	...
	void gta_data_service::rebuild_cache()
	{
			...
			else if (const auto file_str = path.string(); file_str.contains("weapon") && !file_str.contains("vehicle") && path.extension() == ".meta")
			{
				rpf_wrapper.read_xml_file(path, [&exists, &weapons, &mapped_weapons, file_str, &rpf_wrapper](pugi::xml_document& doc) {
					...
					for (const auto& item_node : items)
					{
						...
						if (hash == RAGE_JOAAT("WEAPON_STRICKLER")) // Gen9 exclusive
							continue;
		...
		g_fiber_pool->queue_job([&] {
			...
			{
				std::strncpy(item.m_display_manufacturer,
				    HUD::GET_FILENAME_FOR_AUDIO_CONVERSATION(item.m_display_manufacturer),
				    sizeof(item.m_display_manufacturer));
				std::strncpy(item.m_display_name,
				    HUD::GET_FILENAME_FOR_AUDIO_CONVERSATION(item.m_display_name),
				    sizeof(item.m_display_name));
		...
		g_thread_pool->push([this, peds = std::move(peds), vehicles = std::move(vehicles), weapons = std::move(weapons), weapon_components = std::move(weapon_components)] {
			...
			{
				m_weapons_cache.version_info.m_game_build     = g_pointers->m_game_version;
				m_weapons_cache.version_info.m_online_version = g_pointers->m_online_version;
