...
namespace big
{
	...
	void tunables_service::run_script()
	{
		while (true)
		{
			...
			if (m_cache_file.up_to_date(memory::module("").timestamp()))
			{
			...
			if (!m_script_started)
			{
				...
				SCRIPT::REQUEST_SCRIPT_WITH_NAME_HASH(RAGE_JOAAT("tunables_registration"));

				if (SCRIPT::HAS_SCRIPT_WITH_NAME_HASH_LOADED(RAGE_JOAAT("tuneables_processing")) && SCRIPT::HAS_SCRIPT_WITH_NAME_HASH_LOADED(RAGE_JOAAT("tunables_registration")))
				{
					m_num_tunables = gta_util::find_script_program(RAGE_JOAAT("tunables_registration"))->m_global_count - TUNABLE_BASE_ADDRESS;

					uint64_t args[] = {6, 27}; // TODO: check args

					int id = BUILTIN::START_NEW_SCRIPT_WITH_NAME_HASH_AND_ARGS(RAGE_JOAAT("tuneables_processing"), (Any*)args, sizeof(args) / 8, DEFAULT_STACK_SIZE);
					...
					SCRIPT::SET_SCRIPT_WITH_NAME_HASH_AS_NO_LONGER_NEEDED(RAGE_JOAAT("tunables_registration"));
	...
	void tunables_service::save()
	{
		...
		*(uint32_t*)data_ptr = m_tunables.size();
		...
		m_cache_file.set_header_version(memory::module("").timestamp());
