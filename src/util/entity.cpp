...
namespace big::entity
{
	...
	void delete_entity(Entity& ent, bool force)
	{
		...
		if (auto ptr = g_pointers->m_handle_to_ptr(ent))
		{
			if (ptr->m_net_object)
			{
		...
		{
			for (auto obj : pools::get_all_props_ptr_array())
			{
				auto object = g_pointers->m_ptr_to_handle(obj);
				if (!object)
					break;
			...
			for (auto veh : pools::get_all_vehicles_ptr_array())
			{
				auto vehicle = g_pointers->m_ptr_to_handle(veh);
				if (!vehicle)
					break;
	...
	std::vector<Entity> get_entities(bool vehicles, bool peds, bool props, bool include_self_veh)
	{
		...
		if (vehicles)
		{
			for (auto vehicle : pools::get_all_vehicles_ptr_array())
			{
				if (!vehicle || (!include_self_veh && vehicle == gta_util::get_local_vehicle()))
					continue;

				target_entities.push_back(g_pointers->m_ptr_to_handle(vehicle));
			}
		}

		if (peds)
		{
			for (auto ped : pools::get_all_peds_ptr_array())
			{
				if (!ped || ped == gta_util::get_local_ped())
					continue;

				target_entities.push_back(g_pointers->m_ptr_to_handle(ped));
			}
		}

		if (props)
		{
			for (auto prop : pools::get_all_props_ptr_array())
			{
				if (!prop)
					continue;

				target_entities.push_back(g_pointers->m_ptr_to_handle(prop));
			}
		}
	...
	Entity get_entity_closest_to_middle_of_screen(rage::fwEntity** pointer, std::vector<Entity> ignore_entities, bool include_veh, bool include_ped, bool include_prop, bool include_players)
	{
		...
		auto include_pool = [&](auto& pool) {
			for (const auto ptr : pool())
				if (ptr)
					update_closest_entity(g_pointers->m_ptr_to_handle(ptr), ptr);
		};

		if (include_veh)
			include_pool(pools::get_all_vehicles_ptr_array);

		if (include_ped)
			include_pool(pools::get_all_peds_ptr_array);

		if (include_prop)
			include_pool(pools::get_all_props_ptr_array);
		...
		if (include_players)
		{
			for (auto player : g_player_service->players() | std::ranges::views::values)
			{
				if (player->get_ped())
				{
					Ped handle = g_pointers->m_ptr_to_handle(player->get_ped());
	...
	void force_remove_network_entity(rage::CDynamicEntity* entity)
	{
		if (!entity->m_net_object)
			return;

		force_remove_network_entity(entity->m_net_object->m_object_id, entity->m_net_object->m_ownership_token);
	}

	void force_remove_network_entity(std::uint16_t net_id, int ownership_token)
	{
		char buf[0x200]{};
