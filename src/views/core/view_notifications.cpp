...
namespace big
{
	...
	float draw_notification(float start_pos, ImDrawList* dl, const notification& notif)
	{
		...
		dl->AddRectFilled({(float)*g_pointers->m_resolution_x - 360.f, 10.f + start_pos},
		    ...
		    3357612055);
		dl->AddRectFilledMultiColor({(float)*g_pointers->m_resolution_x - 360.f, 10.f + start_pos},
		    {(float)*g_pointers->m_resolution_x - 255.f, start_pos + 45.f + total_size},
		    fadeBegin,
		    fadeEnd,
		    fadeEnd,
		    fadeBegin);
